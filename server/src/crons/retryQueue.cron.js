...
const task = cron.schedule(
    ...
    async () => {
        if (isRunning) {
            console.warn("queue-retry skipped: previous run still executing");
            return { skipped: true };
        }
        ...
        try {
            ...
            for (const event of events) {
                try {
                    switch (event.eventType) {
                        case "daily-snapshot":
                            await dailySnapshotQueue.add(event.eventType, event.payload);
                            break;
                        case "order-queue":
                        default:
            ...
            console.error("queue-retry fatal error:", err);
            throw err;
        } finally {
            isRunning = false;
        }
